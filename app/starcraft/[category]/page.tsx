...
export async function generateMetadata({
  ...
  const items = await getServerGameCategoryEquipment("starcraft", category);
  if (items.length === 0) {
    return { title: "카테고리를 찾을 수 없습니다 | 템빨" };
  }

  return buildGameCategoryMetadata("starcraft", category, items.length);
}

export default async function StarCraftCategoryPage({ params }: PageProps) {
  const { category } = await params;
  return <GameEquipmentCategoryHub game="starcraft" category={category} />;
}
