...
export async function generateMetadata({
  ...
  const items = await getServerEquipmentByCategory(category);
  if (items.length === 0) {
    return { title: "카테고리를 찾을 수 없습니다 | 템빨" };
  }

  return buildEquipmentCategoryMetadata(category, items.length);
}
...
export default async function EquipmentCategoryPage({ params }: PageProps) {
  ...
  const jsonLd = buildEquipmentCategoryJsonLd(category, items.length);
