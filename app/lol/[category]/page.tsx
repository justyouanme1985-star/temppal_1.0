...
export async function generateMetadata({
  ...
  const items = await getServerGameCategoryEquipment("lol", category);
  if (items.length === 0) {
    return { title: "카테고리를 찾을 수 없습니다 | 템빨" };
  }

  return buildGameCategoryMetadata("lol", category, items.length);
}

export default async function LoLCategoryPage({ params }: PageProps) {
  const { category } = await params;
  return <GameEquipmentCategoryHub game="lol" category={category} />;
}
